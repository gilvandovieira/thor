...
import { asc, count, db, desc, eq, excluded, exists, gt, gte, param, rowNumber, sql, sum } from "@gilvandovieira/thor"
...
export const UpsertRawEvent = db
  .insert(rawEvents)
  .values({
  ...
  .run()
  .compilePrepared(SQLiteDialect)

export const UpsertDailyMetric = db
  .insert(dailyMetrics)
  .values({
...
export const UpdateImportRun = db
  .update(importRuns)
  .set({
  ...
  .where(eq(importRuns.id, param("runId", Schema.String)))
  .run()
  .compilePrepared(SQLiteDialect)

export const FindEvent = db
  .select({
...
export const groupedMetrics = db
  .select({
    sourceId: rawEvents.sourceId,
...
export const highValueMetrics = db
  .select({
    sourceId: rawEvents.sourceId,
    day: rawEvents.occurredDay,
    eventType: rawEvents.eventType,
...
export const clearDailyMetrics = db.delete(dailyMetrics).run().compilePrepared(SQLiteDialect)
