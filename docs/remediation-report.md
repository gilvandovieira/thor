...
| P0.5 | `.one()`/`.maybeOne()` cardinality | **Confirmed, fixed for SELECT; documented for DML** | Memoized SELECT cardinality-probe IR is capped at `LIMIT 2`, preserving tighter user limits. Portable DML cannot be capped this way: `RETURNING.one()`/`.maybeOne()` refine after execution, which is now explicitly tested and documented. `cardinality-probe.test.ts`, `cardinality-observability.test.ts`, `execution.test.ts`, `invalid-shapes.test.ts`. |
