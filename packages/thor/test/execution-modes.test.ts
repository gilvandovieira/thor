...
import { CapabilityError, Database, MySQLDialect, db, pg, withMode } from "@gilvandovieira/thor"
...
describe("execution modes (spec §15.13, §15.17)", () => {
  ...
  it("trusted mode still strictly decodes rows", async () => {
    const driver = rowsAt()
    const [row] = await run(db.select({ at: events.at }).from(events).all(), withMode(FakeDatabaseLayer(driver), "trusted"))
    expect(row!.at).toBeInstanceOf(Date)
  })

  it("unsafe mode skips decoding and returns raw driver values", async () => {
    const driver = rowsAt()
    const [row] = await run(db.select({ at: events.at }).from(events).all(), withMode(FakeDatabaseLayer(driver), "unsafe"))
  ...
  it("unsafe mode still binds parameters and reaches the driver for supported queries", async () => {
    ...
    const result = await run(
      db.insert(events).values({ at: new Date("2026-02-02T00:00:00Z") }).run(),
