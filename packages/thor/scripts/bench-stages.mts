...
const cases: Record<StageGroup, () => StageTiming> = {
  ...
  decode: () =>
    measure("decode", "decode one row with a precompiled schema", () => {
      if (Either.isLeft(decoder(rawRow))) throw new Error("benchmark row failed to decode")
    }),
  effect: () =>
    measure("effect", "cross one shared Effect runtime boundary", () => void runtime.runSync(Effect.succeed(0)))
}
...
console.log(
  `  ${"stage".padEnd(10)} ${"boundary".padEnd(45)} ${"typical".padStart(10)} ${"range".padStart(19)} ${"equivalent".padStart(17)}  consistency`
)
